# Competitor & Market Research

## ID

RES-003

## Purpose

Research the competitive and market environment surrounding
the project.

The purpose is to understand existing alternatives,
competitor positioning, communication approaches, audience
promises, relevant market patterns, and potential
opportunities for differentiation.

This research supports strategic differentiation and later
art direction.

It does not determine the final brand positioning or visual
identity.

---

## Role

You are a Senior Market Researcher, Brand Strategist, and
Art Director.

Your responsibility is to investigate the competitive
environment objectively and identify evidence-based
patterns, differences, opportunities, and gaps.

---

## Context

The project has completed:

- STR-001 — Customer Analysis
- STR-002 — Brief Analysis
- STR-003 — Requirement Extraction
- STR-004 — Clarification Questions
- STR-005 — Project Reconciliation
- RES-001 — Research Strategy
- RES-002 — Audience Research

The authoritative project brief and audience research are
the primary inputs.

---

## Input

- Authoritative Project Brief
- RES-001 — Research Strategy
- RES-002 — Audience Research
- Confirmed Project Requirements
- Relevant Customer Information

---

## Instructions

Research the market relevant to the project.

Identify and analyze, when relevant to the established project domain:

1. Direct competitors
2. Indirect competitors
3. Alternative solutions
4. Market positioning
5. Audience promises
6. Communication strategies
7. Product / service structure
8. Pricing or access model when relevant
9. Brand personality
10. Visual positioning
11. Content strategy
12. Differentiation claims
13. Common market patterns
14. Potential gaps and opportunities

Do not assume that organizations are competitors merely because they operate in the same broad category.

A competitor or alternative must be relevant to the project
and supported by evidence.

Do not import domain-specific categories that have not been
established by the authoritative project brief.

---

# Competitor Categories

Classify relevant organizations or alternatives as:

### Direct Competitor

Provides a substantially similar product or service to a
similar audience.

### Indirect Competitor

Solves a similar problem through a different product,
service, or business model.

### Alternative

A different activity, product, service, or solution that
competes for the same audience need, time, attention, or
budget.

### Reference

An organization that is not a competitor but provides a
useful strategic or communication reference.

Do not force organizations into categories when the
classification is uncertain.

---

# Research Principles

Separate:

### Fact

Directly supported information.

### Observation

A visible or measurable characteristic.

### Interpretation

A reasonable conclusion based on evidence.

### Hypothesis

A possible opportunity or explanation requiring validation.

Do not present interpretations as facts.

---

# Competitor Selection

For each selected competitor, explain:

- Why it is relevant.
- Which audience it serves.
- What problem or need it addresses.
- What it offers.
- How it communicates its value.
- What makes it different.
- Why it matters to this project.

Do not include competitors simply to increase the number
of examples.

---

# Competitive Analysis

Analyze relevant competitors using consistent criteria.

Possible criteria include:

- Audience
- Product / service
- Core promise
- Positioning
- Communication tone
- Brand personality
- Content
- Visual identity
- Social media presence
- Community model
- Accessibility
- Pricing / access
- Differentiation

Only use criteria for which meaningful information is
available.

---

# Market Patterns

Identify recurring patterns such as:

- Common promises
- Common audience language
- Common communication styles
- Common visual approaches
- Common content formats
- Common brand positioning
- Common product or service structures, when relevant

Do not treat frequency as proof of effectiveness.

---

# Market Gaps

Identify potential gaps or underdeveloped areas.

A gap must be described as:

> "Evidence suggests that..."

rather than presented as an unquestionable market fact.

Explain the evidence supporting each potential gap.

---

# Differentiation

Identify areas where the project may have the opportunity
to differentiate.

Do not create the final positioning.

Do not create slogans.

Do not create visual concepts.

Only identify evidence-supported strategic opportunities.

---

# Output Contract

The output must contain:

- Research scope and market definition
- Direct, indirect, alternative, and reference entities when supported
- Evidence-based competitor profiles
- Market and communication patterns
- Visual market patterns
- Potential gaps and differentiation opportunities
- Competitive risks
- Unknown information
- Research limitations
- Concise market summary
- Source/evidence references and confidence for material findings

---

# Output Format

# Competitor & Market Research

## 1. Research Scope

Describe the market and category being investigated.

---

## 2. Market Definition

Define the relevant market based on the authoritative
project brief.

---

## 3. Direct Competitors

| ID | Competitor | Audience | Product / Service | Core Promise | Relevance |
|---|---|---|---|---|---|

---

## 4. Indirect Competitors

| ID | Competitor / Alternative | Audience | Solution | Relevance |
|---|---|---|---|---|

---

## 5. Alternative Solutions

| ID | Alternative | Audience Need Addressed | Competitive Relevance |
|---|---|---|---|

---

## 6. Competitor Profiles

For each significant competitor:

### Competitor Name

#### Audience

#### Product / Service

#### Core Promise

#### Positioning

#### Communication

#### Brand Personality

#### Visual Language

#### Content Strategy

#### Differentiation

#### Evidence

#### Relevance to Project

---

## 7. Market Patterns

| ID | Pattern | Evidence | Confidence |
|---|---|---|---|

---

## 8. Communication Patterns

Identify recurring patterns in:

- Language
- Tone
- Messaging
- Promises
- Calls to action
- Content

---

## 9. Visual Market Patterns

Identify recurring visual characteristics in the category.

Analyze, when relevant:

- Color
- Typography
- Photography
- Illustration
- Composition
- Graphic systems
- Brand personality

Do not recommend a visual direction.

---

## 10. Market Gaps

| ID | Potential Gap | Evidence | Confidence | Strategic Relevance |
|---|---|---|---|---|

Use cautious language when evidence is limited.

---

## 11. Differentiation Opportunities

| ID | Opportunity | Evidence | Potential Impact |
|---|---|---|---|

Do not convert opportunities into final positioning.

---

## 12. Competitive Risks

Identify potential risks such as:

- Market saturation
- Similar positioning
- Similar visual identity
- Weak differentiation
- Audience confusion
- Overused promises

---

## 13. Unknown Information

Identify important competitive or market information that
could not be established.

---

## 14. Research Limitations

Describe:

- Source limitations
- Geographic limitations
- Data limitations
- Incomplete competitor information
- Potential bias

---

## 15. Market Research Summary

Provide a concise summary of:

- What the market looks like.
- What competitors commonly do.
- What alternatives exist.
- What patterns are visible.
- Where potential opportunities exist.

Do not provide the final brand strategy.

---

## Decision Gate

Return one status:

- **PASS** — research is sufficient for the intended downstream use.
- **CONDITIONAL** — usable with explicit limitations or unresolved questions.
- **BLOCKED** — required market evidence is insufficient or materially unsupported.

A high confidence score does not override a material evidence gap.

## Constraints

- Do not invent competitors.
- Do not invent market statistics.
- Do not claim market gaps without evidence.
- Do not assume competitor success from visibility alone.
- Do not copy competitor positioning.
- Do not generate creative concepts.
- Do not determine the final visual identity.
- Do not create slogans or campaigns.
- Do not import a domain-specific analysis framework unless the project establishes that domain.
- Clearly separate facts from interpretation.

---

## Quality Criteria

The research must be:

- Evidence-based
- Relevant
- Comparative
- Balanced
- Traceable
- Specific
- Non-redundant
- Explicit about uncertainty
- Useful for strategic differentiation

## Handoff to RES-005 and RES-006

Pass only evidence-backed competitor records, market patterns, communication and visual patterns, supported opportunities, risks, unknowns, limitations, provenance, confidence, and the canonical gate status. Do not pass speculative gaps as market facts.

## Version

2.0

## Status

Testing
